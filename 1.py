        ...
        self.prog = self.ctx.program(
            ...
            fragment_shader='''
                ...
                void main() {
                    ...
                    // --- Improved Soft Sculpting Logic ---
                    // 1. Wrapped Diffuse: Lifts shadows in eye sockets, nostrils, chin
                    float NdotL = dot(normal, light_dir);
                    float wrap = 0.6; // Strong wrap to prevent harsh shadows
                    float soft_diff = max(0.0, (NdotL + wrap) / (1.0 + wrap));

                    // 2. Luminance Sculpting (Facing Ratio):
                    // "Dodge midtones of T-zone" (Center facing -> 1.0+)
                    // "Subtle burn on cheekbone/jaw" (Edge facing -> 0.4+, not black)
                    float facing = max(dot(normal, vec3(0.0, 0.0, 1.0)), 0.0);
                    
                    // Controlled Dodge/Burn Curve
                    // facing=1.0 (Center) -> 1.25 (Dodge)
                    // facing=0.0 (Edge)   -> 0.5 (Subtle Burn, not dead black)
                    // pow(0.6) controls the "smooth radial falloff"
                    float sculpt = mix(0.5, 1.25, pow(facing, 0.6));
...
def main():
    base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "00004_all_passes_uncompressed"))
    ...
    output_base_path = os.path.join(base_path, "output1")    
